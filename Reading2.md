...
* Given a training set of N example input/output pairs (x<sub>1</sub>, y<sub>1</sub>), (x<sub>2</sub>,y<sub>2</sub>), ..., (x<sub>n</sub>,y<sub>n</sub>) where each y<sub>i</sub> was generated by an unknown function y = f(x), discover a function h that approximates the true function f.
#### 18.3.1 The Decision Tree Representation
* A decision tree represents a function that takes as input a vector of attribute values and returns a single output value, a "decision"
